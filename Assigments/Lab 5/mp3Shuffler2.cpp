...
{
  //Data
  string objective = "Objective: Read song names from a text file named songs.txt, randomly select one song that has not been played in the last five, and output its song to the user.\n";
  ...
  deque<list> lastSongs; // collection of 5 "already played" song names
  ...
  fin.close();
  if (nSongs > 0) // to check for empty file
  {
  ...
  else cout << "Sorry, it seems that there are no songs in the file songs.txt" << endl;
}//main
...
void outputSong(deque<list>& lastSongs, string* song, int nSongs)
{
  ...
  list aSong; // (object of data type list) a temporary object to store a non lastly played song 
...
{
  //Data
  int i; // (integer) counter to traverse list "lastSongs"
  bool recentlyPlayed = false; // (boolean) is true if a song has been recently played (within 5 last songs)

  //Traverse collection of lastSongs to check for if song has been already played in the last 5
  for (i = 0; i < lastSongs.size(); i++)
  {
    if (song == lastSongs[i].name) recentlyPlayed = true;
  }
